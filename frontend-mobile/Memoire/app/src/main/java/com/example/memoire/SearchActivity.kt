    ...
    private fun fetchUsersFromApi(query: String) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val response = RetrofitClient.instance.searchProfiles(query)
                if (response.isSuccessful) {
                    val searchResponse = response.body() // This is of type SearchResponse?
                    if (searchResponse != null && searchResponse.results.isNotEmpty()) {
                        fullUserList = searchResponse.results // Get the list of ProfileDTO from 'results'

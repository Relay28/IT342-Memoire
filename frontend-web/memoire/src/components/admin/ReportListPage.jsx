...
const ReportsListPage = () => {
  ...
  const fetchReports = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/reports/getAll', {
        method: 'GET',
        headers: {
  ...
  const executeAction = async () => {
    ...
    try {
      ...
      switch (confirmAction) {
        case 'report-approve':
          response = await fetch(`http://localhost:8080/api/reports/${selectedReport.id}/status?status=APPROVED`, {
            method: 'PUT',
            headers: {
              'Authorization': `Bearer ${authToken}`,
              'Content-Type': 'application/json'
        ...
        case 'report-reject':
          response = await fetch(`http://localhost:8080/api/reports/${selectedReport.id}/status?status=REJECTED`, {
            method: 'PUT',
            headers: {
              'Authorization': `Bearer ${authToken}`,
              'Content-Type': 'application/json'
        ...
        case 'report-delete':
          response = await fetch(`http://localhost:8080/api/reports/${selectedReport.id}`, {
            method: 'DELETE',
            headers: {
              'Authorization': `Bearer ${authToken}`
            }
